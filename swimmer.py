...
class Swimmer:
    ...
    def calcParamagneticMoment(self, ext_field):
        magnetic_field_para1 = ext_field
        magnetic_field_para2 = ext_field - (1/alpha)*(3*np.sqrt(3)*np.sin(self.theta - np.pi/3) + 2*np.cos(self.theta))*np.array([0, 1, 0])
        #print("1: {}".format(magnetic_field_para1))
        #print("2: {}".format(magnetic_field_para2))

        self.paramagnetic_moment = gamma * magnetic_field_para1
        #self.paramagnetic_moment = gamma * magnetic_field_para2
    ...
    def calcTorque(self, ext_field):
        ...
        #b_all = alpha*b_ext \
        #        + b_dd \
        #        + gamma * (b_ext[1] - (3*np.sqrt(3)*np.sin(self.theta - np.pi/3)+2*np.cos(self.theta))/alpha) * b_para

        b_all = alpha*b_ext + b_dd + gamma*b_para*ext_field[1]

        self.torque = np.cross(self.permanent_magnetic_moment, b_all)
    ...
    def gradientDescent(self, x0, ext_moment):
        ...
        while(True):
            rate = 0.001 / (1 + count)
            delta = self.gradPotential(ext_moment, x)
...
axes[1].set_xticks(np.linspace(-2*np.pi, 2*num_cycle*np.pi, 2*(num_cycle)+1))
#axes[1].set_xticks(np.linspace(-np.pi/3, np.pi/3, 5))
#axes[1].set_xlim(-np.pi/4, np.pi/4)
theta_arr = np.linspace(-2*np.pi, 2*num_cycle*np.pi, 1000*(num_cycle+1))
